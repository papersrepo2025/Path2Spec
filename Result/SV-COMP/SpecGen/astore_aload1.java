class astore_aload1 {
  ...
  public static void func() {
    ...
    /*@
      ...
      @ maintaining boolean_array != null && byte_array != null && char_array != null &&
      @             short_array != null && int_array != null && long_array != null &&
      @             double_array != null && float_array != null;
      @ maintaining boolean_array.length == size && byte_array.length == size &&
      @             char_array.length == size && short_array.length == size &&
      @             int_array.length == size && long_array.length == size &&
      @             double_array.length == size && float_array.length == size;
      @ maintaining (\forall int j; 0 <= j && j < i;
      @                boolean_array[j] == (j % 2 == 0) &&
      @                byte_array[j] == (byte) j &&
      @                char_array[j] == (char) j &&
      @                short_array[j] == (short) j &&
      @                int_array[j] == j &&
      @                long_array[j] == (long) j &&
      @                double_array[j] == (double) j &&
      @                float_array[j] == (float) j);
      @ decreases size - i;
      @*/
